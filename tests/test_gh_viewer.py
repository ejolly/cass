...
class TestGHSubmissionSchemaV12:
    ...
    def test_schema_has_new_columns(self, db_conn):
        cols = db_conn.execute("DESCRIBE gh_submissions").fetchall()
...
class TestViewerConfig:
    ...
    def test_gh_grades_excluded(self):
        assert "gh_grades" in EXCLUDED_TABLES
...
class TestGHEnrichedQueries:
    ...
    def populated_db(self, db_conn):
        ...
        # GH students
        from cass.models import GHStudentInfo
        ...
        db.save_gh_assignments(
            [
                GHAssignment(id=1, slug="hw-01", title="Homework 01"),
                GHAssignment(id=2, slug="hw-02", title="Homework 02"),
            ]
        )
...
class TestGHGradebook:
    ...
    def test_gradebook_unmatched_student_uses_gh_name(self, gradebook_db):
        ...
        db.save_gh_students(
            [
                GHStudentInfo(login="charlie-gh", name="Charlie Brown"),
            ]
        )
        db.save_gh_submissions(
            [
