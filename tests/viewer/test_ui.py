...
def ui_conn() -> sqlite_utils.Database:
    """In-memory SQLite with a realistic schema for viewer UI tests."""
...
class TestTableRendering:
    async def test_table_list(self, user: User, ui_conn: sqlite_utils.Database) -> None:
...
class TestSidebarNavigation:
    async def test_navigation(self, user: User, ui_conn: sqlite_utils.Database) -> None:
    ...
    async def test_editable_badges(
        self, user: User, ui_conn: sqlite_utils.Database
    ) -> None:
        ...
        def page() -> None:
            for t in tables:
                ...
                with ui.row().mark(f"row-{name}"):
                    ...
                    ui.badge("editable" if editable else "view-only").mark(
                        f"badge-{name}"
                    )
...
class TestEditHandler:
    async def test_updates_db_and_pending(
        self, user: User, ui_conn: sqlite_utils.Database
    ) -> None:
    ...
    async def test_non_pushable_no_pending(
        self, user: User, ui_conn: sqlite_utils.Database
    ) -> None:
    ...
    async def test_renders(self, user: User, ui_conn: sqlite_utils.Database) -> None:
        ...
        def page() -> None:
            ...
            n_assignments = sum(
                ...
                or any(
                    ch.get("field", "").startswith("_a") for ch in c.get("children", [])
                )
            )
...
class TestPendingAndRevert:
    async def test_badge_updates(
        self, user: User, ui_conn: sqlite_utils.Database
    ) -> None:
    ...
    async def test_revert_restores_db(
        self, user: User, ui_conn: sqlite_utils.Database
    ) -> None:
        ...
        track_change(
            pending, table, {"canvas_id": 1}, "name", "Homework 1", "Changed Name"
        )
        assert pending_count(pending) == 1
...
class TestGridUtils:
    async def test_find_in_container(
        self, user: User, ui_conn: sqlite_utils.Database
    ) -> None:
    ...
    async def test_find_empty_container(
        self, user: User, ui_conn: sqlite_utils.Database
    ) -> None:
    ...
    async def test_find_none_ref(self, user: User) -> None:
        """find_grid() returns None when container ref is None."""
        result = find_grid({"ref": None})
        assert result is None

    async def test_reload(self, user: User, ui_conn: sqlite_utils.Database) -> None:
